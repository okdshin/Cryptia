...
public:
	...
	//using FigureList = std::deque<BaseType>;

	BasicBigNatural(BaseType num);
    explicit BasicBigNatural(const std::string& num_str);
	
	static auto GenerateRandomly(std::function<BaseType ()> rand, unsigned int figure_num) -> BasicBigNatural {
		BasicBigNatural res(0);
		for(unsigned int i = 0; i < figure_num; ++i){
			res.figure_list_.push_back(rand());
		}
		return res;
	}
...
template<BaseType BASE>
auto BasicBigNatural<BASE>::Base()const -> BaseType {
	return BASE_;
}
